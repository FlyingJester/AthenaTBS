...
static void athena_try_add_breadth_position(int x, int y, int d, const struct Athena_Unit *unit, const struct Athena_Field *field, 
...
static void athena_breadth_positions_iter(const struct Athena_Unit *unit, const struct Athena_Field *field,
    ...
    if(!pos)
        ...
        athena_try_add_breadth_position(pos->x + 1, pos->y, pos->distance, unit, field, in_list, out_list, dead_list);
        athena_try_add_breadth_position(pos->x - 1, pos->y, pos->distance, unit, field, in_list, out_list, dead_list);
        athena_try_add_breadth_position(pos->x, pos->y + 1, pos->distance, unit, field, in_list, out_list, dead_list);
        athena_try_add_breadth_position(pos->x, pos->y - 1, pos->distance, unit, field, in_list, out_list, dead_list);
